...
Screen every candidate before synthesis. A red flag is a reason to revise or reject the shape, not a request to add another wrapper.
...
Information leakage makes several modules depend on the same internal decision. A representation, policy, or protocol detail appears in more than one place, so changing it requires coordinated edits.

Public re-exports of transport, framework, persistence, or wire types are common leakage. Parse external data into domain types behind the interface. Keep storage schemas and protocol details private unless interoperability is the interface's purpose.
...
Group code around domain knowledge and ownership. Methods that run at different times can belong to one module when they protect the same decisions.
...
Remove it or move responsibility to the module that can complete the operation. Keep a forwarding boundary only when it adds policy, adaptation, isolation, or a distinct abstraction.
