#!/usr/bin/env bash
# Append a well-formed row to a decision-trail log (TSV).
...
# with a single quote. This log is meant to be read in spreadsheets, so
# attacker-controlled evidence (PR titles, filenames, generated text)
# must not become formula execution when a reviewer opens the file.
clean() {
	local v
	v=$(printf '%s' "$1" | tr '\t\n\r' '   ')
	case "$v" in
