...
interface InspectorDashboard {
	getRows(): readonly InspectorRow[];
	nowMs(): number;
	subscribe(listener: () => void): () => void;
}

function inspectorRowElapsedSeconds(row: InspectorRow, now: number): number | undefined {
	if (row.startedAt === undefined) return undefined;
	return Math.max(0, Math.round(((row.finishedAt ?? now) - row.startedAt) / 1000));
}

interface InspectorTranscripts {
...
const PANEL_COPY: InspectorCopy = { emptyMessage: "No panel children active" };
...
function computeInspectorBodyLines(
	dashboard: InspectorDashboard,
	transcripts: InspectorTranscripts,
	selectedIndex: number,
	width: number,
...
export function renderInspector(
	...
	const elapsed = inspectorRowElapsedSeconds(selectedRow, dashboard.nowMs());
	...
	// Last line: Key help
	const helpText = `←→/tab child · ↑↓ PgUp PgDn scroll · f follow [${state.follow ? "ON" : "OFF"}] · esc close${copy.helpSuffix ?? ""}`;
...
export class InspectorComponent implements Component {
	...
	private readonly dashboard: InspectorDashboard;
	private readonly transcripts: InspectorTranscripts;
	private readonly tui: RenderRequester;
	private readonly theme: DashboardTheme;
	private readonly onClose: () => void;
	private readonly onAbort?: () => void;
	private readonly text: TerminalText;
	private readonly copy: InspectorCopy;
	...
	ctx.ui
		.custom<void>(
			(tui, theme, _kb, done) => {
				...
				component = new InspectorComponent(dashboard, transcripts, tui, theme, close, onAbort, text, options.copy);
				return component;
			},
			{
				overlay: true,
