...
test("removes its listener after the timer resolves", async () => {
	...
	signal.addEventListener = ((...args: Parameters<AbortSignal["addEventListener"]>) => { added++; return originalAdd(...args); }) as AbortSignal["addEventListener"];
	signal.removeEventListener = ((...args: Parameters<AbortSignal["removeEventListener"]>) => { removed++; return originalRemove(...args); }) as AbortSignal["removeEventListener"];
