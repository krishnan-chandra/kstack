...
export async function runFastImplement(request: FastImplementRequest, role: ResolvedRole, initialCwd: string, fx: FastRunEffects): Promise<FastImplementOutcome> {
	...
	if (request.workLocation === "worktree") { const planned = await planManagedWorktree(initialCwd, request.task, fx.exec); if (!planned.ok) return { status: "failed", error: planned.error }; const created = await createManagedWorktree(planned.plan, fx.exec); if (!created.ok) return { status: "failed", error: created.error }; cwd = created.plan.path; branch = created.plan.branch; }
	...
	} catch (error) { return { status: "failed", error: error instanceof Error ? error.message : String(error), branch, cwd }; } finally { if (temp) rmSync(temp, { recursive: true, force: true }); }
}
