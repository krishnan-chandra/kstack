...
export default function fastImplementExtension(pi: ExtensionAPI): void {
	...
	async function run(request: FastImplementRequest, ctx: ExtensionCommandContext): Promise<void> {
		...
		active = new AbortController(); ctx.ui.setStatus("fast-implement", "fast-implement: implementing…");
		try { const outcome = await runFastImplement(request, role.role, ctx.cwd, { exec: (command, args, options) => pi.exec(command, args, options), signal: active.signal }); pi.sendMessage({ customType: "fast-implement", content: outcome.status === "completed" ? outcome.output : `${outcome.error}\nRetained workstream: ${outcome.cwd ?? ctx.cwd}${outcome.branch ? ` (${outcome.branch})` : ""}`, display: true, details: { status: outcome.status, branch: outcome.branch } }); ctx.ui.notify(outcome.status === "completed" ? `Fast implementation completed on ${outcome.branch}.` : `Fast implementation ${outcome.status}; inspect the retained workstream.`, outcome.status === "completed" ? "info" : "error"); } finally { active = undefined; ctx.ui.setStatus("fast-implement", undefined); }
	}
