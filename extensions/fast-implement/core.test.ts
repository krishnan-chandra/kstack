...
test("parses bounded fast implementation requests", () => {
	...
	if (result.ok)
		assert.deepEqual(result.request, { task: "Add a test", workLocation: "worktree", changeKind: "feature" });
...
test("preserves quotes and contractions in the raw task suffix", () => {
	...
	if (quoted.ok)
		assert.deepEqual(quoted.request, { task: "Fix the narrow bug", workLocation: "current", changeKind: "bug-fix" });
});
