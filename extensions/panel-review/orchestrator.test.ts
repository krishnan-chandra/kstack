...
describe("runPanel", () => {
	it("caps concurrency and returns results in panel order", async () => {
		...
		const panel = await runPanel(specs, 2, async (spec) => {
			...
			running--;
			return { status: "completed", label: spec.label, model: spec.model, output: spec.label, usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, turns: 1 } };
		});
		assert.ok(peak <= 2, `peak concurrency ${peak}`);
		// Deterministic order despite D finishing before A.
		assert.deepEqual(panel.results.map((r) => r.label), ["A", "B", "C", "D"]);
	...
	it("a failed reviewer does not cancel siblings", async () => {
		const panel = await runPanel(specs.slice(0, 2), 2, async (spec): Promise<ReviewerResult> => {
			...
			return { status: "completed", label: spec.label, model: spec.model, output: "ok", usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, turns: 1 } };
		});
